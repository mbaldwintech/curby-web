...
export const ColorPicker = ({ value, defaultValue = '#000000', onChange, className, ...props }: ColorPickerProps) => {
  ...
  useEffect(() => {
    if (value) {
      ...
      setAlpha((color.alpha() !== undefined ? color.alpha() : 1) * 100);
    }
  }, [value]);
...
export const ColorPickerSelection = memo(({ className, ...props }: ColorPickerSelectionProps) => {
  ...
  // Update position indicator when saturation/lightness changes externally
  useEffect(() => {
    if (!isDragging) {
      // Convert saturation (0-100) to x position (0-1)
      const x = saturation / 100;

      // Convert lightness (0-100) to y position (0-1)
      // Reverse the calculation from handlePointerMove
...
export const ColorPickerPresetPicker = ({ presets, ...props }: ColorPickerPresetsProps) => {
  const { setHue, setSaturation, setLightness, setAlpha } = useColorPicker();

  return (
    <div className={cn('flex flex-row gap-2 flex-wrap justify-center', props.className)} {...props}>
      {presets?.map((preset) => {
        const color = Color(preset);
        return (
          <button
            key={preset}
            type="button"
            ...
            onClick={() => {
              const hsl = color.hsl().array();
              const alpha = (color.alpha() !== undefined ? color.alpha() : 1) * 100;
