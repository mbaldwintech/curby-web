...
export interface ItemReviewTableProps {
  defaultFilters?: Filters<ItemReview>;
  onRowClick?: (itemReview: ItemReview) => void;
  rowActionSections?: CurbyTableRowAction<ItemReview>[][];
  toolbarLeft?: React.ReactNode;
  toolbarRight?: React.ReactNode;
  height?: number;
  maxHeight?: number;
  extraColumns?: CustomColumnDef<ItemReview>[];
}
    ...
    () => [
      ...
      buildColumn('reviewOutcomeAction', 'Review Outcome Action', {
        cell: ({ row }) =>
          ...
          ) : row.original.reviewOutcomeAction === 'item_removed' ? (
            <Badge className="text-red-600 bg-red-100 px-1.5">Item Removed</Badge>
          ) : row.original.reviewOutcomeAction === 'item_restored' ? (
            <Badge className="text-green-600 bg-green-100 px-1.5">Item Restored</Badge>
          ) : row.original.reviewOutcomeAction === 'open_user_review' ? (
            <Badge className="text-yellow-600 bg-yellow-100 px-1.5">Open User Review</Badge>
          ) : (
      ...
      buildColumn('appealReviewOutcomeAction', 'Appeal Review Outcome Action', {
        cell: ({ row }) =>
          row.original.appealReviewOutcomeAction === 'no_action' ? (
            <Badge className="text-muted-foreground bg-muted/20 px-1.5">No Action</Badge>
          ) : row.original.appealReviewOutcomeAction === 'item_restored' ? (
            <Badge className="text-green-600 bg-green-100 px-1.5">Item Restored</Badge>
          ) : row.original.appealReviewOutcomeAction === 'user_warning' ? (
            <Badge className="text-yellow-600 bg-yellow-100 px-1.5">User Warning</Badge>
          ) : (
